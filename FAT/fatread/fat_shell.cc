...
namespace
{
    ...
    {
        int fd;
        if (!check_integer("close", args[0], &fd))
            return;
    ...
    void do_lsdir(const std::vector<std::string> &args)
    {
        ...
        for (const AnyDirEntry &entry : entries)
        {
            ...
            {
                std::cout << std::setw(12) << file_name << " " << std::setw(10) << "--"
                          << " " << std::setw(16) << "directory";
            }
            else
            {
    ...
    {
        int fd, count, offset;
        if (!check_integer("pread fd", args[0], &fd))
            return;
        if (!check_integer("pread count", args[1], &count))
            return;
        if (!check_integer("pread offset", args[2], &offset))
            return;
    ...
    {
        int fd, count, offset;
        if (!check_integer("preadandsave fd", args[0], &fd))
            return;
        if (!check_integer("preadandsave size", args[1], &count))
            return;
        if (!check_integer("preadandsave offset", args[2], &offset))
            return;
