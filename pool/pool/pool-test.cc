...
void usage(const char *name) {
    std::cout << "usage: " << name << " short --- for short subset of tests\n";
    std::cout << "   OR: " << name << " long --- for more complete set of tests\n";
}
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "short") == 0) {
        test_run_in_order(1, 2, false, false);
        test_run_in_order(1, 3, false, true);
        test_run_in_order(2, 3, false, false);
        test_run_in_order(2, 3, true, true);
        test_submit_then_wait_then_submit(2, 2);
        test_wait_for_from_future_tasks(2);
    } else if (strcmp(argv[1], "long") == 0) {
        test_run_in_order(1, 1, false, false);
        test_run_in_order(1, 2, false, false);
